...
//    }
    ...
    public int  maxProfit(int[] prices){
        ...
        for (int i = 1; i < prices.length; i++) {
            //第 i 个元素未操作的状态的值一定等于第 i - 1 个元素未操作的状态的值
            dp[i][0] = dp[i - 1][0];
            //第 i 个元素第一次买入的状态的值等于下面两种情况的最大值：1. 在第 i 个元素进行第一次买入。 2.不操作，因为之前已经进行了第一次买入
            dp[i][1] = Math.max(dp[i][0] - prices[i], dp[i - 1][1]);
            //第 i 个元素第一次卖出的状态的值等于下面两种情况的最大值：1. 在第 i 个元素进行第一次卖出。 2.不操作，因为之前已经进行了第一次卖出
            dp[i][2] = Math.max(dp[i][1] + prices[i], dp[i - 1][2]);
            //第 i 个元素第二次买入的状态的值等于下面两种情况的最大值：1. 在第 i 个元素进行第二次买入。 2.不操作，因为之前已经进行了第二次买入
            dp[i][3] = Math.max(dp[i][2] - prices[i], dp[i - 1][3]);
            //第 i 个元素第二次卖出的状态的值等于下面两种情况的最大值：1. 在第 i 个元素进行第二次卖出。 2.不操作，因为之前已经进行了第二次卖出
